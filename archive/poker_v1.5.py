...
def double_up(winnings, deck):
    """ダブルアップゲームを実行する関数
    winnings: 今回のゲームで獲得したボーナスチップ
    deck: 使用中のトランプデッキ
    """
    console.print("\n[bold gold1]★★★ ダブルアップチャンス！ ★★★[/bold gold1]")
    
    while True:
        # デッキの残りが少なくなったら新しく作り直す（念のため）
        if len(deck) < 5:
            deck = make_deck()
            random.shuffle(deck)
        ...
        # 4. 勝敗判定
        # ポーカーと同じく、同じ数字の場合は「引き分け（プレイヤーの負け、または仕切り直し）」にしますが、
        # ここでは分かりやすく「同じ数字なら引き分け（もう一回）」にします。
...
def play_game(player_chips):
    ...
    if result == "勝ち":
        ...
        # --- ここから追加 ---
        # 役なし(0倍)で勝つことは基本的にないですが、念のため1枚以上獲得している場合のみ発動
        if winnings > 0:
            # ダブルアップゲームを呼び出し、最終的な獲得額を受け取る
            winnings = double_up(winnings, deck)
            
        # 最終的なチップの計算（ベットした分 + 確定した獲得分）
